...
var i = 0;
for(i; i<todayPhoto.length; i++){

    str[i] = '<img src="'+todayPhoto[i].img+'"> ';
    string[i] += todayPhoto[i].title;
}

wrap.innerHTML = str[0];
wrap2.innerHTML = str[1];
wrap3.innerHTML = str[2];
Str.innerHTML = string[0];
Str2.innerHTML = string[1];
Str3.innerHTML = string[2];
...
front.addEventListener("click", function(event){
    if(event.target.id='front'){
        if(count < todayPhoto.length-3){
            count++;
            i = count;
            wrap.innerHTML = str[i];
            wrap2.innerHTML = str[i+1];
            wrap3.innerHTML = str[i+2];
            Str.innerHTML = string[i];
            Str2.innerHTML = string[i+1];
            Str3.innerHTML = string[i+2];
            now.innerHTML = count+1+"/6";
        }
    }
});
